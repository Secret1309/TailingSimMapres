...
function FlowIndicator({ from, to, count = 5, speed = 0.4 }: {
    ...
    // Gold = Yellow, Copper = Brown
    const isCopper = tailingType === TAILING_TYPES.COPPER || tailingType.includes("Tembaga");
    const dotColor = isCopper ? "#8b4513" : "#fbbf24";
    const emissiveColor = isCopper ? "#a0522d" : "#fbbf24";
...
// 2. Mixer & Molder — with spinning motor
function MixerMolder() {
    ...
    return (
        <group>
            ...
            <SpinningMotor position={[0, 2.8, 0]} speed={2.5} />
            ...
            <SmallLabel position={[1.5, 3.4, 0.5]} text="BINDER IN" />
...
function CuringChamber() {
    ...
    return (
        <group>
            ...
            {/* Smoke from chimney */}
            <SmokeParticles position={[6.5, 3.1, 0]} />
            ...
            />
            <SmallLabel position={[7.2, 1.0, 0]} text="PRODUK ✓" />
        </group>
    );
}
...
function EquipmentGroup({ autoRotate }: { autoRotate: boolean }) {
    ...
    return (
        <group ref={groupRef}>
            ...
            <FlowIndicator from={[-4.0, 0.5, 0]} to={[-1.25, 0.5, 0]} count={5} speed={0.35} />
            <FlowIndicator from={[1.25, 0.5, 0]} to={[4.0, 0.5, 0]} count={5} speed={0.35} />
