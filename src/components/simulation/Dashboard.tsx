...
export function Dashboard() {
    ...
    const COLORS = ['#374151', '#6b7280', '#9ca3af', '#d1d5db', '#d97706'];
    ...
    const opexBreakdownData = economicResults ? (() => {
        const rawData = [
            ...
            { name: 'Pra-Perlakuan', value: +(economicResults.opexBreakdown.preTreatmentCost / 1e9).toFixed(2), fill: '#d97706' },
        ];
                                    ...
                                    <div className="h-[350px] w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={massBalanceResult.sensitivityData} margin={{ top: 5, right: 40, left: 40, bottom: 25 }}>
                                                ...
                                                <YAxis yAxisId="left" width={80} label={{ value: 'Output (ton/tahun)', angle: -90, position: 'insideLeft', offset: -10 }} />
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="rounded-lg bg-gray-50 p-4 text-sm text-gray-700 border border-gray-200">
                            <p>
                                ...
                                • <strong>OPEX</strong>: Biaya binder (NaOH/Na₂SiO₃), energi listrik, tenaga kerja, pemeliharaan, dan <strong>biaya pra-perlakuan tailing</strong> (destruksi sianida / netralisasi sulfida).<br />
                        ...
                        {/* Karakteristik Fisikokimia & Produksi */}
                        {(() => {
                            const tailing = DEFAULT_TAILING[tailingType];
                            const isGold = tailingType === TAILING_TYPES.GOLD;
                            return (
                                <div className={`rounded-xl border-2 p-6 shadow-sm ${isGold ? 'border-red-300 bg-red-50/30' : 'border-amber-300 bg-amber-50/30'}`}>
                                    <div className="mb-4 flex items-center gap-3">
                                        <span className={`text-lg ${isGold ? 'text-red-600' : 'text-amber-600'}`}>
                                            {isGold ? '☣️' : '⚗️'}
                                        </span>
                                        <h3 className="font-semibold text-gray-800">Karakteristik Fisikokimia & Produksi</h3>
                                        <span className={`rounded-full px-3 py-0.5 text-xs font-semibold ${isGold ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                                            {isGold ? 'Logam Berat & Sianida' : 'Sulfida & AMD'}
                                        </span>
                                    </div>
                                    <div className="space-y-4">
                                        <div>
                                            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Profil Kimia Tailing</p>
                                            <p className="mt-1 text-sm text-gray-700">{tailing.chemicalProfile}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Dampak terhadap Produksi Biogeopolimer</p>
